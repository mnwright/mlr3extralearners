#' @title Regression Rf With Hierarchical Shrinkage Learner
#' @author mnwright
#' @name mlr_learners_regr.ranger_hshrink
#'
#' @description
...
#'
#' @templateVar id regr.ranger_hshrink
#' @template learner
#'
#' @references
...
#' @template example
#' @export
LearnerRegrRangerHshrink = R6Class("LearnerRegrRangerHshrink",
  inherit = LearnerRegr,
  public = list(
    #' @description
    ...
    initialize = function() {
      ...
      super$initialize(
        id = "regr.ranger_hshrink",
        packages = "ranger",
        feature_types = c("logical", "integer", "numeric", "factor", "ordered"),
        predict_types = c("response"),
        param_set = param_set,
        properties = c("oob_error"),
        man = "mlr3extralearners::mlr_learners_regr.ranger_hshrink",
    ...
    .train = function(task) {
      ...
      if (is.numeric(lambda) & length(lambda) == 1) {
        ...
        res = sapply(lambda, function(ll) {
          ...
          oob_pred = rowMeans(oob_idx * preds, na.rm = TRUE)
...
.extralrns_dict$add("regr.ranger_hshrink", LearnerRegrRangerHshrink)
